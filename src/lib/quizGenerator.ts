import { QuizQuestion, StudentNote, LearnerPersona, LectureQuizAnalysis } from '../types';

/**
 * Curated high-yield mastery questions for standard benchmark lecture notes.
 */
const CURATED_NOTE_QUIZZES: Record<string, QuizQuestion[]> = {
  'note-misc-nda': [
    {
      id: 'nda-q1',
 ...
 */
export function synthesizeMasteryQuizFromContent(
  noteContent: string,
  noteTitle: string,
  ...
  while (questions.length < Math.max(5, count) && templateIdx < defaultTemplates.length) {
    ...
    questions.push({
      ...
      explanation: t.expl,
      topic: t.top,
      difficulty: t.diff as 'easy' | 'moderate' | 'hard'
    });
  }
...
export async function fetchAIQuizQuestions(
  ...
  return synthesizeMasteryQuizFromContent(
    note.personalisedNotes || note.content || '',
    note.title,
    persona,
    count
  );
