...
export const LectureNotesStudio: React.FC<LectureNotesStudioProps> = ({
  ...
  // Launch Post-Reading Quiz strictly generated by Gemini AI with resilient curriculum fallback
  const handleStartQuiz = async () => {
    ...
    try {
      const questions = await fetchAIQuizQuestions(activeNote, persona, 5);
      if (Array.isArray(questions) && questions.length >= 3) {
        setQuizQuestions(questions);
        // Persist to the active note
        activeNote.quiz = {
          id: `quiz-${activeNote.id}-${Date.now()}`,
          title: `Mastery Quiz: ${activeNote.title}`,
          topic: activeNote.title,
          questions,
          createdAt: new Date().toISOString()
        };
      } else {
        const fallback = getOrGenerateQuizQuestions(activeNote, persona);
        setQuizQuestions(fallback);
      }
    } catch (err: any) {
      console.warn('AI quiz generation notice, using synthesized curriculum questions:', err);
      const fallback = getOrGenerateQuizQuestions(activeNote, persona);
      setQuizQuestions(fallback);
    } finally {
      setQuizLoading(false);
  ...
  const handleSubmitQuiz = async () => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        if (data && data.difficultyBreakdown) {
          setQuizAnalysis(data);
          return;
        }
      }
      // If server doesn't provide structured breakdown, evaluate locally
      const localAnalysis = evaluateQuizPerformance(
        activeNote?.title || 'Lecture Checkpoint',
        quizQuestions,
        userAnswers,
        persona
      );
      setQuizAnalysis(localAnalysis);
    } catch (err) {
      console.warn('Failed to analyze quiz performance via server, evaluating locally:', err);
      const localAnalysis = evaluateQuizPerformance(
        activeNote?.title || 'Lecture Checkpoint',
        quizQuestions,
        userAnswers,
