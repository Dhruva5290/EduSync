...
const fakeData_1 = require("../src/mock/fakeData");
const supabaseUsers_1 = require("../src/server/supabaseUsers");
const quizGenerator_1 = require("../src/lib/quizGenerator");
async function handler(req, res) {
    ...
    try {
        ...
        if (path.includes('/api/auth/public-users') || path.endsWith('/public-users')) {
            const cloudUsers = await (0, supabaseUsers_1.loadUsersFromCloud)();
            const all = [...fakeData_1.FAKE_USERS];
        ...
        if (path.includes('/api/auth/login') && req.method === 'POST') {
            ...
            let user = fakeData_1.FAKE_USERS.find(u => (u.username && u.username.toLowerCase() === loginId) ||
            ...
            if (!user) {
                user = await (0, supabaseUsers_1.findUserInCloud)(loginId);
            }
            if (!user) {
                res.status(401).json({
        ...
        if ((path.includes('/api/users') || path.endsWith('/users')) && !path.includes('/api/users/') && req.method === 'POST') {
            ...
            await (0, supabaseUsers_1.persistUserToCloud)(newUser);
        ...
        if (path.includes('/api/users/') && req.method === 'DELETE') {
            ...
            if (userId) {
                await (0, supabaseUsers_1.deleteUserFromCloud)(userId);
            }
        ...
        if (path.includes('/api/admin/provision-department') && req.method === 'POST') {
            ...
            if (Array.isArray(users)) {
                for (const u of users) {
                    (0, supabaseUsers_1.persistUserToCloud)(u).catch(() => { });
                }
            }
        ...
        if (path.includes('/api/ai/quiz/generate-mastery-quiz') && req.method === 'POST') {
            ...
            if (process.env.GEMINI_API_KEY) {
                try {
                    const { generateMasteryQuizAI } = await Promise.resolve().then(() => __importStar(require('../src/server/gemini')));
            ...
            if (cleanT.includes('nda') || cleanT.includes('selection') || cleanT.includes('ssb')) {
                const { getOrGenerateQuizQuestions } = await Promise.resolve().then(() => __importStar(require('../src/lib/quizGenerator')));
                ...
                questions = (0, quizGenerator_1.synthesizeMasteryQuizFromContent)(noteContent, title, learnerProfile, count);
            }
            res.status(200).json({
        ...
        if (path.includes('/api/ai/quiz/analyze-performance') && req.method === 'POST') {
            ...
            if (process.env.GEMINI_API_KEY) {
                try {
                    const { analyzeQuizPerformanceAI } = await Promise.resolve().then(() => __importStar(require('../src/server/gemini')));
            ...
            const localAnalysis = (0, quizGenerator_1.evaluateQuizPerformance)(quizTitle, questions, answersMap, learnerProfile);
