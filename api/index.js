...
async function persistUserToCloud(user) {
  const sb = getClient();
  if (!sb) return false;
  try {
...
async function findUserInCloud(loginId) {
  const sb = getClient();
  ...
  try {
    ...
    const matched = cloudUsers.find(
      (u) => u.username && u.username.toLowerCase() === normalized || u.email && u.email.toLowerCase() === normalized || u.institutionalId && u.institutionalId.toLowerCase() === normalized || u.id && u.id.toLowerCase() === normalized || u.name && u.name.toLowerCase() === normalized
    );
    return matched || null;
  } catch (err) {
  ...
  try {
    ...
    if (path.includes("/api/auth/login") && req.method === "POST") {
      ...
      let user = FAKE_USERS.find(
        (u) => u.username && u.username.toLowerCase() === loginId || u.email && u.email.toLowerCase() === loginId || u.institutionalId && u.institutionalId.toLowerCase() === loginId || u.name && u.name.toLowerCase() === loginId
      );
      if (!user) {
    ...
    if ((path.includes("/api/users") || path.endsWith("/users")) && !path.includes("/api/users/") && req.method === "POST") {
      ...
      const newUser = {
        ...
        academicYear: targetRole === "student" ? academicYear || "1st Year" : void 0,
        designation: targetRole !== "student" ? designation || "Faculty" : void 0,
        phone: phone || "+91 98765 43210",
        gpa: targetRole === "student" ? Number(gpa || 8) : void 0,
        status: "active",
    ...
    res.status(200).json({ status: "ok", time: (/* @__PURE__ */ new Date()).toISOString(), path });
  } catch (err) {
