...
exports.db = void 0;
exports.saveLecturesToDisk = saveLecturesToDisk;
exports.loadLecturesFromDisk = loadLecturesFromDisk;
exports.saveProgressToDisk = saveProgressToDisk;
exports.loadProgressFromDisk = loadProgressFromDisk;
exports.saveUsersToDisk = saveUsersToDisk;
exports.loadUsersFromDisk = loadUsersFromDisk;
exports.saveNotesToDisk = saveNotesToDisk;
exports.loadNotesFromDisk = loadNotesFromDisk;
const fakeData_1 = require("../mock/fakeData");
const classsarthiSeed_1 = require("./classsarthiSeed");
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
const users_json_1 = __importDefault(require("../../data/users.json"));
const lectures_json_1 = __importDefault(require("../../data/lectures.json"));
const notes_json_1 = __importDefault(require("../../data/notes.json"));
const student_progress_json_1 = __importDefault(require("../../data/student_progress.json"));
const isServerless = Boolean(process.env.VERCEL || process.env.AWS_LAMBDA_FUNCTION_NAME);
const USERS_FILE_PATH = path_1.default.resolve(process.cwd(), 'data', 'users.json');
const NOTES_FILE_PATH = path_1.default.resolve(process.cwd(), 'data', 'notes.json');
const LECTURES_FILE_PATH = path_1.default.resolve(process.cwd(), 'data', 'lectures.json');
const PROGRESS_FILE_PATH = path_1.default.resolve(process.cwd(), 'data', 'student_progress.json');
function saveLecturesToDisk(lectures) {
    if (isServerless)
        return;
    try {
        const dir = path_1.default.dirname(LECTURES_FILE_PATH);
        if (!fs_1.default.existsSync(dir)) {
            fs_1.default.mkdirSync(dir, { recursive: true });
        }
        fs_1.default.writeFileSync(LECTURES_FILE_PATH, JSON.stringify(lectures, null, 2), 'utf-8');
    }
    catch (err) {
        console.error('Error saving lectures to disk:', err);
    }
}
function loadLecturesFromDisk(seed) {
    const fallbackSeed = (Array.isArray(lectures_json_1.default) && lectures_json_1.default.length > 0) ? lectures_json_1.default : seed;
    try {
        if (fs_1.default.existsSync(LECTURES_FILE_PATH)) {
            const content = fs_1.default.readFileSync(LECTURES_FILE_PATH, 'utf-8');
...
function saveProgressToDisk(progress) {
    if (isServerless)
        return;
    try {
        const dir = path_1.default.dirname(PROGRESS_FILE_PATH);
        if (!fs_1.default.existsSync(dir)) {
            fs_1.default.mkdirSync(dir, { recursive: true });
        }
        fs_1.default.writeFileSync(PROGRESS_FILE_PATH, JSON.stringify(progress, null, 2), 'utf-8');
    }
    catch (err) {
        console.error('Error saving progress to disk:', err);
    }
}
function loadProgressFromDisk(seed) {
    const fallbackSeed = (student_progress_json_1.default && typeof student_progress_json_1.default === 'object') ? student_progress_json_1.default : seed;
    try {
        if (fs_1.default.existsSync(PROGRESS_FILE_PATH)) {
            const content = fs_1.default.readFileSync(PROGRESS_FILE_PATH, 'utf-8');
            const parsed = JSON.parse(content);
...
function saveUsersToDisk(users) {
    if (isServerless)
        return;
    try {
        const dir = path_1.default.dirname(USERS_FILE_PATH);
        if (!fs_1.default.existsSync(dir)) {
            fs_1.default.mkdirSync(dir, { recursive: true });
        }
...
function loadUsersFromDisk(seed) {
    const fallbackSeed = (Array.isArray(users_json_1.default) && users_json_1.default.length > 0) ? users_json_1.default : seed;
    try {
        if (fs_1.default.existsSync(USERS_FILE_PATH)) {
            const content = fs_1.default.readFileSync(USERS_FILE_PATH, 'utf-8');
            const parsed = JSON.parse(content);
            if (Array.isArray(parsed) && parsed.length > 0) {
                return parsed;
            }
...
function saveNotesToDisk(notes) {
    if (isServerless)
        return;
    try {
        const dir = path_1.default.dirname(NOTES_FILE_PATH);
        if (!fs_1.default.existsSync(dir)) {
            fs_1.default.mkdirSync(dir, { recursive: true });
        }
        fs_1.default.writeFileSync(NOTES_FILE_PATH, JSON.stringify(notes, null, 2), 'utf-8');
    }
    catch (err) {
        console.error('Error saving notes to disk:', err);
    }
}
function loadNotesFromDisk(seed) {
    const fallbackSeed = (Array.isArray(notes_json_1.default) && notes_json_1.default.length > 0) ? notes_json_1.default : seed;
    try {
        if (fs_1.default.existsSync(NOTES_FILE_PATH)) {
            const content = fs_1.default.readFileSync(NOTES_FILE_PATH, 'utf-8');
            const parsed = JSON.parse(content);
            if (Array.isArray(parsed) && parsed.length > 0) {
...
$$2I = \\int_{0}^{\\pi/2} 1 \\, dx = \\frac{\\pi}{2} \\implies I = \\frac{\\pi}{4}$$`,
    ...
    lectures: loadLecturesFromDisk(classsarthiSeed_1.seedLectures),
    boardCaptures: classsarthiSeed_1.seedBoardCaptures,
    conceptMastery: classsarthiSeed_1.seedConceptMastery,
    lectureProgress: loadProgressFromDisk(classsarthiSeed_1.seedStudentLectureProgress),
    masteryQuizzes: classsarthiSeed_1.seedMasteryQuizzes,
    questionBanks: [...fakeData_1.FAKE_QUESTION_BANKS]
};
