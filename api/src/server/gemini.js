...
exports.buildPersonaPromptInstructions = buildPersonaPromptInstructions;
exports.generateDynamicTopicVideos = generateDynamicTopicVideos;
exports.generateDynamicTopicPracticeQuestions = generateDynamicTopicPracticeQuestions;
exports.generateDynamicTopicQuiz = generateDynamicTopicQuiz;
exports.generateStudyAssistantReply = generateStudyAssistantReply;
exports.researchTopicAndVideosAI = researchTopicAndVideosAI;
exports.summarizeNoteAI = summarizeNoteAI;
exports.generateDetailedTopicNoteAI = generateDetailedTopicNoteAI;
exports.generateFlashcardsAI = generateFlashcardsAI;
exports.generateNoteQuizAI = generateNoteQuizAI;
exports.generatePromptQuizAI = generatePromptQuizAI;
exports.generateClassDiagnosticsAI = generateClassDiagnosticsAI;
exports.generateSyllabusTimelineAI = generateSyllabusTimelineAI;
exports.generateMasteryQuizAI = generateMasteryQuizAI;
exports.analyzeQuizPerformanceAI = analyzeQuizPerformanceAI;
exports.recraftNoteForPersona = recraftNoteForPersona;
exports.personalizeNoteAI = personalizeNoteAI;
exports.askMyClassLectureAI = askMyClassLectureAI;
exports.personalizeLectureNotesFromClassSarthi = personalizeLectureNotesFromClassSarthi;
const genai_1 = require("@google/genai");
const security_1 = require("./security");
const knowledgeBase_1 = require("./knowledgeBase");
let aiInstance = null;
function buildPersonaPromptInstructions(persona) {
    if (!persona) {
 ...
 */
function generateDynamicTopicVideos(topicQuery, subjectCode, subjectName) {
 ...
 */
function generateDynamicTopicPracticeQuestions(topicQuery, subjectCode, subjectName) {
 ...
 */
function generateDynamicTopicQuiz(topicQuery, subjectCode, subjectName) {
    const clean = topicQuery.replace(/[#*`?]/g, '').trim();
    return {
        id: `quiz-${Date.now()}`,
 ...
 */
async function generateStudyAssistantReply(context) {
    ...
    if (isGreeting) {
        const synthesized = (0, knowledgeBase_1.synthesizeIntelligentAcademicResponse)(rawQuery, subjectCode, subjectName, contextResources, context.subject, context.upcomingTimelines);
        return {
...
Return your response in clean JSON format:
{
  ...
  "sources": ["Course Syllabus / Academic Reference"]
    ...
    const { cleanText: sanitizedUserMessage } = (0, security_1.sanitizePromptInput)(context.userMessage || '');
    try {
        ...
        const synthesized = (0, knowledgeBase_1.synthesizeIntelligentAcademicResponse)(sanitizedUserMessage, subjectCode, subjectName, contextResources, context.subject, context.upcomingTimelines);
        return {
 ...
 */
async function researchTopicAndVideosAI(prompt, subject) {
        ...
        const response = await ai.models.generateContent({
            ...
            config: {
                ...
                responseSchema: {
                    ...
                    properties: {
                        summary: { type: genai_1.Type.STRING, description: 'A 2-3 sentence executive synthesis of the note content' },
                        keyTakeaways: {
 ...
 */
async function generateDetailedTopicNoteAI(payload) {
    const { prompt, subject, depth = 'exam_prep', attachedText, documentName } = payload;
    const { cleanText: sanitizedPrompt } = (0, security_1.sanitizePromptInput)(prompt || '');
    const { cleanText: sanitizedAttached } = (0, security_1.sanitizePromptInput)(attachedText || '');
    const apiKey = process.env.GEMINI_API_KEY;
        ...
        const response = await ai.models.generateContent({
            ...
            config: {
                ...
                responseSchema: {
                    type: genai_1.Type.OBJECT,
                    properties: {
                        title: { type: genai_1.Type.STRING, description: 'Clear academic title of the generated note' },
                        content: { type: genai_1.Type.STRING, description: 'Complete structured Markdown note content' },
                        tags: {
                            type: genai_1.Type.ARRAY,
                            items: { type: genai_1.Type.STRING },
                            description: '3-4 relevant topic and subject tags'
                        },
                        summary: { type: genai_1.Type.STRING, description: 'Executive 2-sentence synthesis of note' },
                        keyTakeaways: {
                            type: genai_1.Type.ARRAY,
                            items: { type: genai_1.Type.STRING },
            ...
            config: {
                ...
                responseSchema: {
                    ...
                    items: {
                        type: genai_1.Type.OBJECT,
                        properties: {
                            question: { type: genai_1.Type.STRING, description: 'Clear, concise concept question' },
                            answer: { type: genai_1.Type.STRING, description: 'Accurate, complete explanation or formula' },
                            hint: { type: genai_1.Type.STRING, description: 'Helpful clue without giving away the full answer' },
                            topic: { type: genai_1.Type.STRING, description: 'Specific sub-topic label' }
                        },
 ...
 */
async function generateNoteQuizAI(noteContent, title, learnerProfile, teacherQuestions) {
    const apiKey = process.env.GEMINI_API_KEY;
    ...
    try {
        ...
        const response = await ai.models.generateContent({
            ...
            config: {
                ...
                responseSchema: {
                    type: genai_1.Type.OBJECT,
                    properties: {
                        title: { type: genai_1.Type.STRING, description: 'Engaging title for the quiz' },
                        questions: {
                            type: genai_1.Type.ARRAY,
                            items: {
                                type: genai_1.Type.OBJECT,
                                properties: {
                                    ...
                                    question: { type: genai_1.Type.STRING, description: 'Challenging multiple choice question' },
                                    options: {
                                        type: genai_1.Type.ARRAY,
                                        items: { type: genai_1.Type.STRING },
                                        description: 'Exactly 4 distinct plausible options'
                                    },
                                    correctIndex: { type: genai_1.Type.INTEGER, description: '0-based index of the single correct answer' },
                                    explanation: { type: genai_1.Type.STRING, description: 'Step-by-step conceptual rationale' },
                                    topic: { type: genai_1.Type.STRING, description: 'Topic category' },
                                    difficulty: { type: genai_1.Type.STRING, description: 'easy, moderate, or hard' }
                                },
        ...
        const parsed = JSON.parse(response.text || '{}');
        const aiQuestions = (parsed.questions || []).map((q, i) => ({
                        ...
                        questions: {
                            ...
                            items: {
                                ...
                                properties: {
                                    ...
                                    correctIndex: { type: genai_1.Type.INTEGER },
...
Grade Distribution: ${JSON.stringify(currentAnalytics.gradeDistribution || [])}`,
            config: {
                ...
                responseSchema: {
                    type: genai_1.Type.OBJECT,
                    properties: {
                        aiExecutiveSummary: { type: genai_1.Type.STRING, description: 'Executive summary with bolded key metrics and actionable pedagogical narrative' },
                        keyActionItems: {
                            type: genai_1.Type.ARRAY,
                            items: { type: genai_1.Type.STRING },
                        ...
                        weakTopics: {
                            ...
                            items: {
                                ...
                                properties: {
                                    topic: { type: genai_1.Type.STRING },
                                    errorRate: { type: genai_1.Type.NUMBER, description: 'Estimated percent error rate (e.g. 35)' },
                                    averageScore: { type: genai_1.Type.NUMBER, description: 'Average score on this topic (e.g. 68)' },
                                    affectedStudents: { type: genai_1.Type.NUMBER },
                                    recommendedRemediation: { type: genai_1.Type.STRING },
                                    urgency: { type: genai_1.Type.STRING, enum: ['high', 'medium', 'low'] }
                                },
            ...
            config: {
                ...
                responseSchema: {
                    ...
                    items: {
                        ...
                        properties: {
                            title: { type: genai_1.Type.STRING },
                            type: { type: genai_1.Type.STRING, enum: ['lecture', 'quiz', 'exam', 'practical', 'assignment'] },
                            weekNumber: { type: genai_1.Type.INTEGER },
                            description: { type: genai_1.Type.STRING },
                            topicsCovered: {
                                type: genai_1.Type.ARRAY,
                                items: { type: genai_1.Type.STRING }
                            },
                            weightagePercent: { type: genai_1.Type.INTEGER }
                        },
...
async function generateMasteryQuizAI(noteContent, title, learnerProfile, count = 6) {
    ...
    if (!apiKey) {
        ...
        const { synthesizeMasteryQuizFromContent } = await Promise.resolve().then(() => __importStar(require('../lib/quizGenerator')));
        return {
                            ...
                            questions: {
                                ...
                                items: {
                                    ...
                                    properties: {
                                        ...
                                        explanation: { type: genai_1.Type.STRING },
                                        topic: { type: genai_1.Type.STRING },
                                        difficulty: { type: genai_1.Type.STRING, enum: ['easy', 'moderate', 'hard'] }
                                    },
 ...
 */
async function analyzeQuizPerformanceAI(quizTitle, questions, userAnswers, learnerProfile) {
    ...
    const masteryLevel = percentage >= 85 ? 'Mastered' : percentage >= 60 ? 'Proficient' : 'Needs Review';
            ...
            config: {
                ...
                responseSchema: {
                    ...
                    properties: {
                        ...
                        masteryLevel: { type: genai_1.Type.STRING, enum: ['Mastered', 'Proficient', 'Needs Review'] },
                        keyMisconceptions: {
                            type: genai_1.Type.ARRAY,
                            items: { type: genai_1.Type.STRING }
                        },
                        suggestedTutorTopic: { type: genai_1.Type.STRING },
                        suggestedTutorPrompt: { type: genai_1.Type.STRING }
                    },
 ...
 */
function recraftNoteForPersona(note, persona) {
 ...
 */
async function personalizeNoteAI(noteContent, title, learnerProfile) {
...
- Maintain complete accuracy of all LaTeX formulas ($$...$$).`,
            config: {
                ...
                responseSchema: {
                    type: genai_1.Type.OBJECT,
                    properties: {
                        content: { type: genai_1.Type.STRING, description: 'Complete restructured Markdown text with LaTeX' },
                        summary: { type: genai_1.Type.STRING, description: '1-2 sentence executive conceptual summary' },
                        keyTakeaways: {
                            type: genai_1.Type.ARRAY,
                            items: { type: genai_1.Type.STRING },
...
async function askMyClassLectureAI(question, lecture) {
    const sanitizedQuestion = (0, security_1.sanitizePromptInput)(question).cleanText;
...
4. If a formula or board diagram was drawn by the teacher, provide the exact LaTeX formula and mention the board capture.`,
                ...
                responseSchema: {
                    type: genai_1.Type.OBJECT,
                    properties: {
                        answer: { type: genai_1.Type.STRING, description: 'Direct answer grounded strictly in lecture data with timestamp citation' },
                        timestamp: { type: genai_1.Type.STRING, description: 'Relevant timestamp (e.g. "21:05", "12:48") if applicable' },
                        isGrounded: { type: genai_1.Type.BOOLEAN, description: 'True if answer was present in lecture data, false otherwise' },
                        quoteSnippet: { type: genai_1.Type.STRING, description: 'Exact quote or speech snippet from teacher if available' }
                    },
                    required: ['answer', 'isGrounded']
                }
            }
        });
        const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('AI generation timeout')), 3500));
...
function generateLocalAskMyClassReply(question, lecture) {
    const q = question.toLowerCase();
    // Inertia inquiry
    if (q.includes('inertia')) {
        return {
            answer: 'The teacher explained inertia around 12:48. Dr. Verma defined inertia as the intrinsic resistance of matter to change its velocity, with mass ($m$) serving as the scalar measure. He gave the real-world example of passengers jerking forward on a braking metro train because their bodies maintain velocity.',
            timestamp: '12:48',
    ...
    if (q.includes('graph') || q.includes('curve') || q.includes('diagram')) {
        return {
            answer: 'The teacher explained the diagram around 21:05 and the friction graph around 31:42. The diagram plotted on the board illustrates the vector decomposition of gravity on an inclined plane alongside the static vs kinetic friction threshold: $f_s \\le \\mu_s N$.',
            timestamp: '21:05',
            timelineEventId: 'tl-4',
            isGrounded: true,
    ...
    if (q.includes('homework') || q.includes('assignment') || q.includes('task')) {
        return {
            answer: 'Yes, the teacher gave homework at timestamp 42:10. Dr. Verma assigned problems 4 through 9 from Chapter 5 of HC Verma on connected pulley systems and friction blocks, due Friday at 5:00 PM.',
            timestamp: '42:10',
    ...
    if (q.includes('first law') || q.includes('1st law')) {
        return {
            answer: 'The teacher explained Newton\'s First Law around 05:32: Every body continues in its state of rest or uniform motion in a straight line unless acted upon by a net external force ($\\sum \\vec{F} = 0 \\iff \\vec{v} = \\text{constant}$).',
            timestamp: '05:32',
    ...
    const matchedEvent = lecture.timeline.find(t => q.split(' ').some(word => word.length > 3 && t.title.toLowerCase().includes(word)));
    if (matchedEvent) {
        return {
    ...
    return {
        answer: `This specific question was not explicitly covered in today's class on "${lecture.title}". The teacher focused on Newton's First Law (05:32), Inertia (12:48), Free Body Diagrams (21:05), Friction Numericals (31:42), and Pulley Homework (42:10).`,
        isGrounded: false
    };
}
// =========================================================================
...
// =========================================================================
async function personalizeLectureNotesFromClassSarthi(lecture, weakConcepts = [], studentHistory) {
